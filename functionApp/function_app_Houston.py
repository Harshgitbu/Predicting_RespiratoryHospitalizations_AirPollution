...
@app.timer_trigger(schedule="0 */5 * * * *", arg_name="myTimer", run_on_startup=True,
              use_monitor=True) 
def project_timer_trigger_Houston(myTimer: func.TimerRequest) -> None:
    try:
        if myTimer.past_due:
        ...
        lat, lon = 29.7601, -95.3701  # Coordinates for Boston
...
def cdc_trigger_Houston(cdcTimer: func.TimerRequest) -> None:
    try:
        logging.info("🏥 Starting CDC ingestion...")
        seven_days_ago = (datetime.datetime.utcnow() - timedelta(days=7)).strftime('%Y-%m-%d')
        cdc_url = f"{cdc_base_url}?$limit=1000&$where=jurisdiction='TX' AND weekendingdate >= '{seven_days_ago}'"
